...
ten_perc_N = floor(nrow(Data) / 10)
K_wt = rep(NA, 100) # placeholder of N of community of Walktrap
K_fg = rep(NA, 100) # placeholder of N of community of Fast-greedy

for (i in 1:100) {
  
  set.seed(1234 + i)
  sampled_cases <- Data[sample(x = 1:nrow(Data), size = ten_perc_N), ]
  ...
  ggm_igraph <- graph_from_adjacency_matrix(adjmatrix = abs(omega_mat), weighted = TRUE, mode = "undirected")
  ...
  K_wt[i] = length(ggm_wt)
  K_fg[i] = length(ggm_fg)
}
```

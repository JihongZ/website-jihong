...
-   **Mix synthetic AI responses with real human responses** to create a larger dataset, then test the measurement tool’s reliability and validity across both groups.
...
OpenAI recently publishes GPT-5 and claimed that it provides better [data analysis and research capabilities](https://academy.openai.com/home/clubs/work-users-ynjqu/resources/intro-gpt-5).

> GPT-5 is better at research and analysis.
> Summarize large datasets quickly and accurately.
...
GPT-5 is OpenAI’s latest frontier multimodal model focused on high‑accuracy reasoning, data analysis, and tool use. It supports long context, structured outputs (JSON), and improved grounding via tools (code execution, retrieval, and function calling). Typical uses include research assistance, analytics over uploaded files, and robust code generation/refactoring. You can access it through the Responses API using the model name `gpt-5` (text) and pair it with features like structured outputs and file inputs for reliable, reproducible workflows.

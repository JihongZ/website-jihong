...
```
This fits `prestige` as a linear function of `education`, and tests for nonconstant
...
The arguments `dta` and `meanmod` are defined in the environment of the function, but the call to `lm` looks for them in the global environment, and they are
therefore invisible when `lm` is called.

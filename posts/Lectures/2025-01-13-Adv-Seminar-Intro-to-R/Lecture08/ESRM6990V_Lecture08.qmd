...
-   Objectives:
    ...
    -   "Distinguish when to use or not to use `aes()` to change graph's aesthetics (e.g. colors, shapes)."
...
2.  The variables (columns of `data.frame`) that will be *mapped* to different **aesthetics** of the graph (e.g. axis, colors, shapes, etc.)
...
Make a [boxplot](https://www.data-to-viz.com/caveat/boxplot.html) that shows the distribution of `children_per_woman` (y-axis) for each `world_region` (x-axis). (Hint: using `geom_boxplot()`)
...
3. Here are some common types of graph you may want to do for particular situations: 
    ...
    - Look at variation within a single variable using *histograms* (`geom_histogram()`) or, less commonly (but [quite useful](https://towardsdatascience.com/what-why-and-how-to-read-empirical-cdf-123e2b922480)) *empirical cumulative density function* plots (`stat_ecdf`). 

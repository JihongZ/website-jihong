...
3.  Randomized Complete Block Design with R Programming[^1]
:::
...
## A little bit statistics [^purdue]

-  Can partition $\mathrm{SS}_{\mathrm{T}}=\sum \sum\left(y_{i j}-\bar{y}_{. .}\right)^{2}$ into:

$$
\mathrm{SS}_{\mathrm{T}}= b \sum\left(\bar{y}_{i .}-\bar{y}_{. .}\right)^{2}+a \sum\left(\bar{y}_{. j}-\bar{y}_{. .}\right)^{2}+\sum \sum\left(y_{i j}-\bar{y}_{i .}-\bar{y}_{. j}+\bar{y}_{. .}\right)^{2}
$$

-   $\mathrm{SS}_{\mathrm{treatment}}= b \sum\left(\bar{y}_{i .}-\bar{y}_{. .}\right)^{2}$ with $\mathrm{df} = a -1$

-   $\mathrm{SS}_{\mathrm{block}}= a \sum\left(\bar{y}_{. j}-\bar{y}_{. .}\right)^{2}$ with $\mathrm{df} = b -1$
  
-   $\mathrm{SS}_{\mathrm{Residual}}= \sum \sum\left(y_{i j}-\bar{y}_{i .}-\bar{y}_{. j}+\bar{y}_{. .}\right)^{2}$ with $\mathrm{df} = (a-1)(b -1)$

$$
\mathrm{SS}_{\mathrm{T}} = \mathrm{SS}_{\mathrm{treatment}} + \mathrm{SS}_{\mathrm{block}} + \mathrm{SS}_{\mathrm{Residual}}
$$

[^purdue]: Original post can be found [here](https://www.stat.purdue.edu/~xbw/courses/stat514/notes/lec6.blockdesign.pdf)
...
::: callout-important
## Background
An experiment was designed to study the performance of four different detergents in
cleaning clothes. The following “cleanness” readings (higher=cleaner) were obtained with
specially designed equipment for three different types of common stains. Is there a
...
3. **Sum of Squares for treatment**: $SS_{treatment}$ = `{r} (sum(treatment_marginal_Sums^2) / 3 - (sum(detergents[, 2:4]))^2 / 12) |>  as.character()`
...
## Example 4: Performance of Students

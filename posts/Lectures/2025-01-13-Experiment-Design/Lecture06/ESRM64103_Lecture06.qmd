...
3.  **Solomon four group design**: - Features: two treatment groups and two control groups. Only two groups are pretested
...
-   Assumption: " This self-objectification is hypothesized to (a) produce body shame, which in turn leads to restrained eating, and (b) consume attentional resources, which is manifested in diminished mental performance."
...
::: {.column width="50%"}
-   Multitrait-Multimethod Matrix (MTMM):
    -   Example: 3 traits to be measured: c1 (geometry), c2 (algerbra), c3 (reasoning)

...
-   **Total Sum of Squares (SS_total)**: Total variance in the data.
-   **Sum of Squares Between (SS_between)**: Variance due to the model.
-   **Sum of Squares Within (SS_within)**: Variance due to error.
-   Relationship: ( SS\_{total} = SS\_{between} + SS\_{within} )

## Steps in ANOVA

1.  Compute total variability.
2.  Decompose total variability into model-related and error-related components.
3.  Compute the F-statistic: \[ F\_{obs} = \frac{MS_{between}}{MS_{within}} \]
4.  Construct the ANOVA table and draw conclusions.
...
-   **Hypotheses:**
    -   ( H_0: \mu*{Control} =* \mu{G1} = \mu*{G2} =* \mu{G3} )
...
#| eval: true
library(ggplot2)
library(car) # used for leveneTest
```
...
### Visual Inspection of Variance Equality

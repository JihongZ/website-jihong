...
AllDataLong |> 
  group_by(ID) |> 
  summarise(meanHeighID = mean(HeightIN),
            meanAge = mean(Age))
```
...
AllDataLong |> 
  group_by(ID) |> 
  mutate(StudentAvgAge = mean(Age)) |>
  mutate(AgeCbyStu = Age - StudentAvgAge) 
...
AllDataLong$AgeYear = round(x = AllDataLong$AgeC, digits = 2)
View(AllDataLong)
AllDataLong$AgeDecade = signif(x = AllDataLong$Age, digits = 1)
head(AllDataLong)

AllDataLong |> 
  mutate(AgeGroup = ifelse(Age <= 12, 1, ifelse(Age <= 14, 2, 3))) |> 
  

AllDataLong |> 
  mutate(TimeNew = factor(Time, levels = 1:4, labels = c("Pre", "Pre", "Post", "Post"))) 
```

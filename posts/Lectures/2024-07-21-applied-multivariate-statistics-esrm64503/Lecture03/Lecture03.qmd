...
-   [Model for the Variance: Stability of Estimates]{style="color: violet; font-weight:bold"}
    ...
    Variance-Covariance Matrix of Regression Coefficients (standard error variances and residual covariances)

    1.  $$
        \mathbf{\Sigma_{\beta}}=\begin{bmatrix}
        \mathbf{
        \color{tomato}{Var(\beta_1)}, Cov(\beta_1,\beta_2), Cov(\beta_1,\beta_3), \cdots,Cov(\beta_1, \beta_K) \\
        Cov(\beta_2, \beta_1), \color{tomato}{Var(\beta_2)}, Cov(\beta_2,\beta_3), \cdots,Cov(\beta_2, \beta_K) \\
        \dots \\
        \dots \\
...
::: columns
::: {.column width="50%"}
![](figures/paper_sample1.png){fig-align="center"}
:::

::: {.column width="50%"}
![](figures/paper_sample2.png){fig-align="center"}
:::
:::
...
-   Conditional Main Effect of *Motiv* : $(1 + 0.5\times \text{Exam})$
    ...
    -   Simple main effect = 1 if Exam is 0; = 2 if Exam is 1; = 3 if Exam is 4

-   Conditional Main Effect of *Exam* : $(2 + 0.5\times \text{Motiv})$

    -   Simple main effect = 2 if Motiv is 0; = 3 if Motiv is 1; = 4 if Motiv is 4
...
-   $$
    \mathbf{\beta_1^{Senior} = \beta_1+\beta_3}
    $$ $$
    \mathbf{SE_{\beta_1^{Senior}}^2 = Var(\beta_1) + Var(\beta_3) * New  + 2Cov(\beta_1, \beta_3)*New} 
    $$

...
$$
\boldsymbol {\hat \theta_i = \Lambda' \Delta^{-1}(I-\Omega)\Delta^{-1}Y_i}
$$

Then, if we set network scores as

$$
\boldsymbol{\hat\eta_i= W \Delta^{-1}(I-\Omega)\Delta^{-1} Y_i}
$$ Where $\boldsymbol{W}$ is a $1 \times J$ weight matrix suggesting the contribution of items.

When $\boldsymbol{W \approx \Lambda}$, then $\boldsymbol{\hat\eta \approx \hat\theta}$

...
corr_true_est_theta <- function(lambda, J) {
...
corr_true_est_theta(0.7, 6)

dat <- expand.grid(lambda=seq(.1,.9,.1), J = c(6, 12 , 24)) |> 
  as.data.frame() |> 
  rowwise() |> 
  mutate(cor = corr_true_est_theta(lambda = lambda, J = J))

J_labeller <- paste0("J = ", c(6, 12, 24))
names(J_labeller) <- c("6", "12", "24")
ggplot(dat) +
  geom_point(aes(x=lambda, y = cor)) +
  geom_path(aes(x=lambda, y = cor, group = J)) +
  scale_x_continuous(breaks =  seq(.1,.9,.1)) +
  scale_y_continuous(breaks =  seq(.1, 1,.1)) +
  facet_wrap(~ J, nrow = 3, labeller = labeller(J = J_labeller)) +
  labs(x = greekLetters::greeks("lambda"), title = "Expected Correlation between Estimated Factor scores and True Factor Scores") +
  theme_bw()
```

...
**Ae Kyong Jung** takes the ideas of estimating cutscores for contunous latent traits into the area of **Computerized Adaptive Testing (CAT)**, especially item selection procedure. The idea is very naturally, if you can convert the continous latent distribution into discrete distribution using the method presented by Alfonso and Jonathan, you then can use Shannon Entropy for item selection and IRT for theme estimation at the same time. She then performed the comparison between **Shannon entropy** and **D-optimality in MIRT** in terms of item accuracy. The results highlights the D-optimality method in MIRT outperforms Shannon Entropy in the DCM framework. Ae Kong’s work tries to find the balance **diagnostic precision** and **theta estimation accuracy**.

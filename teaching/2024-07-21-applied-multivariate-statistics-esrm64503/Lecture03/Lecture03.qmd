...
6.  Quiz at the end of the class
...
-   [Model for the Variance: Stability of Estimates]{style="color: violet; font-weight:bold"}
    ...
    1.  **Variance-Covariance Matrix of Regression Coefficients** (Diagonal: variances of betas (squared standard errors of betas); Non-diagonal: residual covariances between betas)
...
-   But you won't know what Y is actually supposed to be unless you know where the predictor variables are starting from!

-   Therefore, the intercept is the "YOU ARE HERE" sign in the map of your data... so it should be somewhere in the map\*!
...
-   Interaction can always be evaluated for any combination of categorical and continuous predictors, although
    ...
    2.  In "**ANCOVA**": Continuous predictors ("covariates") do not get to be part of interactions ➡️ make the "homogeneity of regression" assumption
...
::: callout-note
This ANOVA table is displaying **marginal tests** (F-test) for the main effects. Marginal tests are for the main effect only and are not conditional on any interacting variables.
:::

## Getting estimates and standard errors of group means
...
Copy and paste your R syntax and output that calculates the **Senior-Old**'s standard errors of means (use the data and model we use in class). For example, our class syntax for Freshman-New is like:
...
Using `glht` function to simultaneous testing for group means
```{r}
#| output-location: default
...
rownames(effect_model1_matrix) <- c( "beta_Senior_New0", # Conditional Main Effect of Senior given old instruction (New = 0)
                                     "beta_Senior_New1", # Conditional Main Effect of Senior given new instruction (New = 1)
                                     "beta_New_Senior0", # Conditional Main Effect of New Instruction given Senior = 0
                                     "beta_New_Senior1") # Conditional Main Effect of New Instruction given Senior = 1
...
::: panel-tabset
**Instructions**: Answer the following questions based on today's lecture content.
...
a) 4 dummy variables; intercept = grand mean
b) 3 dummy variables; intercept = reference group mean
c) 3 dummy variables; intercept = grand mean
d) 4 dummy variables; intercept = reference group mean

### Question 2
In the model $Y_p = \beta_0 + \beta_1X_1 + \beta_2X_2 + \beta_3X_1X_2$, what does $\beta_1$ represent?

a) Main effect of $X_1$ across all values of $X_2$
b) Simple main effect of $X_1$ when $X_2 = 0$
c) Conditional main effect of $X_1$ depending on $X_2$
d) Interaction effect between $X_1$ and $X_2$
...
a) To increase the model's $R^2$
b) Because the interaction adjusts the main effects
c) To maintain the degrees of freedom
d) To prevent multicollinearity

### Question 5
If you want to test whether a conditional main effect is significantly different from zero, which method would NOT work?

a) Use `glht()` function with contrast matrix
b) Re-center predictors and re-estimate the model
c) Hand calculations using variance-covariance matrix
d) Simply look at the original main effect p-value

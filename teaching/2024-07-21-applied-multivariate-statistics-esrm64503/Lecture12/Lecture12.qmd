...
-   [You will receive 2 extra credit points if you complete the course evaluation.]{style="color: royalblue; font-weight: bold"}

| **Course**       | **Course Name**             | **Survey Open** | **Survey Close** |
|------------------|-----------------------------|-----------------|------------------|
| ESRM 64503 - 001 | Applied Multivariate Statistics  | Nov 20 6:00 AM     | Dec 5 11:59 PM           |
...
library(kableExtra)
library(tidyverse)
library(DescTools) # Desc() allows you to quickly screen data
library(lavaan)
# options(digits = 3)
```
...
-   Exploratory analyses attempt to discover hidden structure in data with minimal user input
    -   Aside from selecting the analysis and estimation method
    
-   Results from exploratory analyses can be misleading
...
-   Sometimes, exploratory analyses are needed
    -   Construct analyses that capitalize on known features of the data
    -   There are better ways to conduct such analyses
    
-   Often, exploratory analyses are not needed
    -   But are conducted anyway—many scale-development reports start with the idea that a construct has a certain number of dimensions
...
-   For example, $\mathbf \Lambda$ is an orthogonal matrix
...
-   Orthogonal matrices are characterized by two properties
    ...
    2.  Each row vector has unit length
...
-   A square matrix $\mathbf{\Sigma}$ can be decomposed into a set of eigenvalues $\boldsymbol{\lambda}$ and a set of eigenvectors $\mathbf e$:
...
-   Each eigenvalue has a corresponding eigenvector
    -   The number equals the number of rows/columns of $\mathbf{\Sigma}$
-   **Principal components analysis** uses eigenvalues and eigenvectors to reconfigure data

## R Code: Eigenvalues and Eigenvectors

-   In R, we can use the `eigen()` function to obtain eigenvalues and eigenvectors of a square matrix (e.g., a correlation matrix).
...
-   These terms will have much greater meaning in principal components analysis
...
where $i$ indexes the rows/columns of the square matrix
...
1.  `tcrossprod`: computes $\mathbf{e}\mathbf{e}^T$
...
::: callout-note
Interpreting the transformation: An eigenvector indicates the direction stretched by the transformation; the corresponding eigenvalue is the factor by which it is stretched.
:::
...
-   In our example, $|\mathbf R| = 1.775 \times 0.224 \approx 0.398$

# PRINCIPAL COMPONENTS ANALYSIS (PCA)

## PCA Overview

1.  **Principal Components Analysis** (PCA) is a method for re-expressing the covariance (or correlation) among a set of variables
...
2.  PCA has two objectives:
    ...
    1.  **Data reduction** — reducing many original variables to a few “components”

        -   Example: You have 100 variables, but only need 7 composite scores that summarize them.
...
-   The goal of PCA is to find a set of [*k*]{.underline} principal components (composite variables) that:
    ...
    -   Accounts for nearly all of the total variance
        -   Total variance = trace of the covariance/correlation matrix
-   If these two goals are accomplished, then the set of [*k*]{.underline} principal components contains almost as much information as the original V variables
    -   Meaning – the components can now replace the original variables in any subsequent analyses

## Questions when using PCA

-   PCA analyses proceed by addressing two questions:
    1.  How many **components** are needed to “adequately” represent the original data?
        -   Example: For a 10-item depression survey, we may need one composite “depression” score
        -   The term “[adequately]{.underline}” is subjective and depends on context
    2.  (once #1 has been answered): What does each component represent?
        -   The term [“represent”]{.underline} is also subjective
        -   Example: With one component, it may represent “general depression level.” What about two or three components?
...
-   PCA usually serves as a means to an end rather than an end in itself
    -   Components (the new variables) are often used in other statistical techniques
        -   Multiple regression/ANOVA
        -   Cluster analysis
-   Unfortunately, PCA is often conflated with Exploratory Factor Analysis
    -   Please avoid using them interchangeably.
...
-   The new components are linear combinations:
    -   We can generate at most V components, the same as the number of observed variables:
...
-   The weights of the components ($e_{jk}$) come from the eigenvectors of the covariance or correlation matrix (component k and variable j)
...
-   Using the eigenvalues and eigenvectors means:
...
-   The R function for principal components is `prcomp()`
...
#| fig-width: 18
#| fig-height: 6
# create the same analysis but with the covariance matrix (for visualization), scale.=FALSE
...
-   To study the etiology of pathological gambling, more variability in responses was needed
...
-   The instrument was administered to a sample of experienced gamblers from a riverboat casino in a Midwestern state
...
-   The Gambling Research Instrument (GRI) used a 6-point Likert scale:
    ...
    -   5: Agree
    -   6: Strongly Agree
-   To meet the assumptions of factor analysis, we will treat these responses as continuous
    -   This is tenuous at best, but common in factor analysis
...
-   Data were collected from two sources:
    ...
    -   1192 college students from a Midwestern state
...
-   Today, we will combine both samples and treat them as homogeneous—one sample of 1304 subjects
...
-   We will retain 4 components (variance accounted for, VAC ≈ 75%)
...
-   F1 relates to the largest number of items (general gambling level?), followed by F2 (specific gambling tendencies?), F3, and F4. It is difficult to know precisely what each factor represents.
...
-   Multiple rotation types exist, but two broad categories dominate discussion:

-   Orthogonal rotations: force factor correlations to zero (axes at 90 degrees). The most prevalent is varimax.

-   Oblique rotations: allow non-zero factor correlations (axes not at 90 degrees). The most prevalent is promax.
...
-   Promax rotation also yields factor correlations:

-   Each factor explains different sets of items
...
-   Today we discussed exploratory factor analysis and found the following:

    1. PCA is often run before EFA
    2. ML-based EFA is often preferable (likelihood-based)
    3. Some constraints in EFA are implicit/hidden
    4. Rotations can fail, making factors hard to interpret

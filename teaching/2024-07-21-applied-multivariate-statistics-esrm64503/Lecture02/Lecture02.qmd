...
```{r}
#| echo: false
wide_marginal_desp |> kable(digits = 3)
```
...
#| results: hide
library(tidyr)
long_marginal_desp <- wide_marginal_desp |> 
...
long_marginal_desp |> kable(digits = 3)
```
...
## Assign values
cor_cov_mat[lower.tri(cor_cov_mat)] <- cor_mat[lower.tri(cor_mat)] # Lower triangle is the pairwise correlation
cor_cov_mat[upper.tri(cor_cov_mat)] <- cov_mat[upper.tri(cov_mat)] # Upper triangle is the covariances
...
-   If you have the covariance matrix (variances and covariances):
    ...
    $$
    r_{Y_1,Y_2}=\frac{S_{Y_1,Y_2}}{S_{Y_1}\times S_{Y_2}}
    $$

-   If you have the correlation matrix and the standard deviations:

    $$
    S_{Y_1, Y_2} = r_{Y_1, Y_2} \times S_{Y_1} \times S_{Y_2}
    $$
...
# Create a function to convert Correlation into Covariance
CorToCov <- function(r, s1, s2){ 
  return(r * s1 * s2)
}

r <- cor(dataSexHeightWeight$heightIN, dataSexHeightWeight$weightLB)
CorToCov(r = r, s1 = sd(dataSexHeightWeight$heightIN), s2 = sd(dataSexHeightWeight$weightLB))
cov(dataSexHeightWeight$heightIN, dataSexHeightWeight$weightLB) 
```

:::{.rmdwarning}
**Exercise**: Write a function to convert the covarance into the correlation.

```{r}
#| eval: false
#| output-location: default

CovToCor <- function(cov, s1, s2){
  r = ADD_YOUR_ANSWER_HERE
  return(r)
}

Cov <- cov(dataSexHeightWeight$heightIN, dataSexHeightWeight$weightLB)
CovToCor(cov = Cov, s1 = sd(dataSexHeightWeight$heightIN), s2 = sd(dataSexHeightWeight$weightLB))
```
...
```{r}
#| output-location: default
plot(x = dataSexHeightWeight$heightIN, 
     y = dataSexHeightWeight$weightLB, 
     xlab = "Height (inches)", 
     ylab = "Weight (LB)")

abline(h = mean(dataSexHeightWeight$weightLB), col = "red")
```
...
summary(model1)$coefficients # regression cofficients table
anova(model1)
```

```{r}
#| echo: false
summary(model1)$coefficients |> kable() # F-statistic table
anova(model1) |> kable()
```
...
summary(model2)$coefficients # regression cofficients table
anova(model2)  # F-statistic table
```
...
summary(model2a)$coefficients  # regression cofficients table
anova(model2a)  # F-statistic table
```
...
summary(model3)$coefficients  # regression cofficients table
...
summary(model4)$coefficients  # regression cofficients table
anova(model4)  # F-statistic table
```
...
summary(model5)$coefficients  # regression cofficients table
anova(model5)  # F-statistic table
```

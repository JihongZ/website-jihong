...
-   **Two types of repeated measures are common:**
    ...
    -   [x] The **order** of treatment administration in a crossover experiment is called a **“sequence.”**
        -   The sequences should be determined a priori and the experimental units are randomized to sequences.
    -   [x] **Time** of a treatment administration is called a **“period.”**
...
-   ✔ **Example**:
    ...
    -   Consider a 3×3 crossover design where a subject receives treatments A, B, and C across three periods. If the subject receives treatment B in Period 1 and treatment A in Period 2, the effect observed in Period 2 may not reflect the pure effect of A—it may be contaminated by the residual effect of B, thus biasing the estimation of treatment A's effectiveness.
...
-   ✔ **However, carryover effects** can bias the interpretation of data analysis, so an investigator should proceed cautiously when implementing a crossover design.
...
-   Recall that in a **between-subjects ANOVA** (i.e., the “usual” ANOVA), we partition total variability into:
...
::: {.column}
![](images/Lecture 11_ Repeated Measure ANOVA-2025-04-06-232317.png)
Generated by [https://www.mermaidchart.com/app/projects](https://www.mermaidchart.com/app/projects)
:::
...
**_Note._** Under the repeated measures ANOVA, we can decompose the total variability:
...
## 1. Think about “between-time” row in ANOVA table
...
## 2. Think about “between-dog” row in ANOVA table
...
## 3. Think about “within-time” row in ANOVA table
...
## 4. Think about “Error” row in ANOVA table
...
data <- data.frame(
  subject = factor(1:6),
  time1 = c(10, 12, 13, 12, 11, 10),
  time2 = c(16, 19, 20, 18, 20, 22),
  time3 = c(25, 27, 28, 25, 26, 27),
  time4 = c(26, 25, 28, 15, 18, 19)
)
...
anova_results <- data_long %>%
  anova_test(dv = histamine, wid = subject, within = time)
...
> Mauchly's test indicated that the assumption of sphericity had been violated, W = 0.621, p = .013. Greenhouse-Geisser corrected results showed a significant effect of time, F(2.34, 11.69) = 6.82, p = .008.

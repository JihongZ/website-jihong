...
3.  To review the whole file, click `</> Code` next to the title of this paper.
    Find `View Source` and click the button.
    Then, you can paste to the newly created Quarto Document.
...
# They store both the values and the levels (categories)
...
table(gender_factor)  # Frequency table
summary(gender_factor)  # Summary statistics

# Converting factors
as.character(gender_factor)  # Back to character
as.numeric(gender_factor)    # To numeric (level numbers)

# Creating factors from numeric data
age_groups <- c(25, 30, 35, 25, 40, 30)
...
# Method 1: Using cut() function to create age groups
...
# Factors are essential for statistical analysis and plotting
# They help R understand categorical variables properly
```
...
-   The data frame is an R object that is a rectangular array of data.
    The variables in the data frame can be any class (e.g., numeric, character) and go across the columns.
    The observations are across the rows.

-   We will start by importing data from a comma-separated values (csv) file.

-   We will use the read.csv() function.
    Here, the argument `stringsAsFactors` keeps R from creating data strings

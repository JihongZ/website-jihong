...
-   A repeated measures ANOVA analyzes data where the same subjects are measured multiple times (a "within-subjects" factor), while a mixed ANOVA combines both a within-subjects factor and a "between-subjects" factor, which has independent groups. 

-   In essence, a mixed ANOVA is a broader category that can be thought of as a more complex repeated measures design, or as a two-way ANOVA with one within-subjects and one between-subjects facto
...
-   Repeated Measure ANOVA (Y:blood concentration of histamine)

| Person | Drug | Time 1 | Time 2 | Time 3 |
|--------|--------|--------|--------|--------|
| Person1 | Drug 1 | $Y_{t_1,p_1}$ | $Y_{t_2,p_1}$ | $Y_{t_3,p_1}$ |
| Person2 | Drug 1 | $Y_{t_1,p_2}$ | $Y_{t_2,p_2}$ | $Y_{t_3,p_2}$ |
| ... | ... | ... | ... | ... |
| Person100 | Drug 3 | $Y_{t_1,p_100}$ | $Y_{t_2,p_100}$ | $Y_{t_3,p_100}$ |
...
::: rmdnote
Sphericity is an assumption for repeated measures ANOVA that states the variances of the differences between all pairs of within-subject conditions are equal. It's a crucial assumption because a violation can lead to an inflated Type I error rate (a false positive).
:::
---
...
- Course evaluation opens at Nov 20 6:00 AM to Dec 5 11:59 PM

- Strongly recommend to make some comments [hopefully positive :)] so that the department will recognize my teaching work

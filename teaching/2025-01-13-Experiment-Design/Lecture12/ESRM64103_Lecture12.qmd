...
-   [x] The relationship between each covariate and the DV should be the same for each level of the IV.
-   [x] *Think of this as an interaction between the IV and covariate.*
-   [x] *We don’t want this in ANCOVA!*
...
➤ **Reliability of Covariates** - ✓ Because the covariates are used in a linear prediction of the DV, no error is assessed or removed from the covariate in the way it is for the DV. - ✓ So, it is assumed that the covariates are measured without any error. ➔ You’ll learn MUCH more about reliability of measures in Dr. Turner’s measurement class! ➔ So, despite the fact that it’s a crucial assumption, we won’t spend much time on it here.
...
-   The $df_{method}$\* is the same as before: *k - 1*, where *k* represents the number of groups.
    ...
    -   [x] In our scenario, we have 3 groups, so the numerator df = 3 − 1 = **2**

-   The $df_{error}$ is different:

    -   [x] *N − k − #covariates*, where *k* is the number of groups and *#covariates* is the number of continuous controls.
    -   [x] In our scenario, if there are 200 students (*N = 200*), 3 groups, and 1 covariate (e.g., IQ), the df is\
        **200 − 3 − 1 = 196**

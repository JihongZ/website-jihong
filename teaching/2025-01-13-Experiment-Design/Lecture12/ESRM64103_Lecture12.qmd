...
#| warning: false
#| code-fold: true
#| code-summary: "Visualization: adjusted differences at common covariate"
#| fig.width: 12
#| fig.height: 6
# Visualizing ANCOVA-adjusted differences at a common covariate value
# Chunk includes a self-contained simulation to create df used below.
library(dplyr)
library(ggplot2)

# -------------------------
# Simulate data to match lecture narrative
# -------------------------
set.seed(2025)
n_students <- 200
...
# Covariate: treat as motivation/verbal-like pretest
pretest <- rnorm(n_students, mean = 65, sd = 6)

# Method-specific intercepts with common slope (homogeneity of regression)
intercepts <- c("lecture" = 20, "hands-on" = 25, "self-paced" = 23)
common_slope <- 0.7
...
df <- tibble(
  ...
  mutate(
    posttest = intercepts[as.character(method)] + common_slope * pretest + error
  )

# -------------------------
# ANCOVA visualization logic
# -------------------------

# Fit common-slope model for ANCOVA logic
ancova_mod <- lm(posttest ~ pretest + method, data = df)

# Pooled slope used in adjusted mean formula
b <- coef(lm(posttest ~ pretest, data = df))["pretest"]

# Grand covariate mean
grand_x <- mean(df$pretest)

# Compute adjusted means at the grand covariate mean (classic ANCOVA adjusted means)
adj_means <- df %>%
  group_by(method) %>%
  summarize(
    ybar = mean(posttest),
  ...
  mutate(adj_mean = ybar - b * (xbar - grand_x))

# Build prediction lines (common slope, method-specific intercepts from ancova_mod)
newx <- data.frame(pretest = seq(min(df$pretest), max(df$pretest), length.out = 100))
pred_lines <- do.call(rbind, lapply(levels(df$method), function(m) {
  tmp <- cbind(newx, method = factor(m, levels = levels(df$method)))
  tmp$yhat <- predict(ancova_mod, newdata = tmp)
  tmp
}))

# Plot: observed points, common-slope lines by group, adjusted mean markers at grand covariate mean
ggplot(df, aes(x = pretest, y = posttest, color = method)) +
  geom_point(alpha = 0.5) +
  geom_line(data = pred_lines, aes(x = pretest, y = yhat, color = method), size = 1) +
  geom_vline(xintercept = grand_x, linetype = "dashed", color = "grey40") +
  geom_point(
    data = adj_means,
    aes(x = grand_x, y = adj_mean, color = method),
    size = 3, shape = 18, show.legend = FALSE
  ) +
  labs(
    x = "Covariate (e.g., motivation / verbal proficiency)",
    y = "Math posttest score",
    color = "Instructional method",
    title = "Instructional-method differences if groups were equivalent on the covariate",
    subtitle = "Points = observed; lines = common-slope fits; diamonds = adjusted means at grand covariate mean"
  ) +
...
ggplot(df, aes(x = height, y = yield, color = genotype)) +
  ...
  theme_minimal(base_size = 14) +
  labs(x = "height", y = "weight", color = "genotype") +
